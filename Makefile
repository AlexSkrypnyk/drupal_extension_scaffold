...
chromedriver-start:
	@if curl -s http://localhost:4444/status | grep -qE '"ready": ?true'; then \
		...
		if [ -z "$$chrome_version" ]; then \
			echo "ERROR: Could not determine the installed Chrome version."; \
			exit 1; \
		fi; \
		mkdir -p .chromedriver; \
		echo "Fetching chromedriver matching Chrome $$chrome_version..."; \
		if ! npx -y @puppeteer/browsers install "chromedriver@$$chrome_version" --path .chromedriver >.chromedriver/install.log; then \
			echo "ERROR: Failed to install chromedriver matching Chrome $$chrome_version."; \
			exit 1; \
		fi; \
		chromedriver_bin=$$(sed -n '$$s/^[^ ]* //p' .chromedriver/install.log); \
		if [ -z "$$chromedriver_bin" ]; then \
			echo "ERROR: Could not determine the chromedriver binary path."; \
			exit 1; \
		fi; \
