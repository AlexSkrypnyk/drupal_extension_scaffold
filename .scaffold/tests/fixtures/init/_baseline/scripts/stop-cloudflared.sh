...
#
# Kills the `cloudflared` process started by `scripts/start-cloudflared.sh`.
# Removes the TUNNEL_URL entry from `.env` so a dead public URL is not
# reported on the next run. Runs even when CLOUDFLARE_TUNNEL is no longer
# set, so cleanup always happens; a no-op when no tunnel is active. The
